...
//                    trigger.setSuccessCondition("$[?(@.props['New status'] == 'TRANSITIONALLY_UP' || @.props['New status'] == 'UP')]");
                    } else {
                ...
                if (alert.getMetric() != null) {
                    condition += format(US, THRESHOLD_CONDITION_FORMAT, alert.getMetric().eventProperty(), alert.getThreshold());
                }
    ...
    public void disable(final AlertEntity alert) {
        if (alertEnabled) {
            final Trigger trigger = new Trigger.Builder().id(alert.getId()).enabled(true).build();
            alertEngineService.send(trigger)
                    .exceptionally(t -> {
